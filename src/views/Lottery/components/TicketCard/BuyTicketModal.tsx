  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        availableSymbol="UERN"
      />
      <div>
        <Tips>{t('1 Ticket = %lotteryPrice% UERN', { lotteryPrice: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your UERN cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t('You will spend: %num% UERN', { num: cakeCosts(val) })}</Final>
      </div>
      <ModalActions>
